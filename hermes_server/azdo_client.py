...
async def get_user_avatar_b64(identity_id: str | None) -> str | None:
    """Fetch and base64-encode a user's avatar image from the AzDO Graph API.

    :param identity_id: Azure DevOps user identity GUID.
    :returns: Base64 data URI string or None if unconfigured or unavailable.
    """
    if not settings.AZDO_PAT or not settings.AZDO_ORGANIZATION_URL or not identity_id:
        return None
    cached = _avatar_cache.get(identity_id)
    if cached is not None or identity_id in _avatar_cache:
        return cached
    try:
        url = f"{settings.AZDO_ORGANIZATION_URL.rstrip('/')}/_apis/graph/avatars/{identity_id}"
...
async def get_user_groups(identity_id: str) -> dict[str, list[str]]:
    """Return AzDO group IDs and display names that this user belongs to.
    ...
    try:
        ...
        url = f"{settings.AZDO_ORGANIZATION_URL.rstrip('/')}/_apis/identities/{identity_id}"
        ...
        if member_of_ids:
            ...
            for i in range(0, len(member_of_ids), batch_size):
                ...
                resolve_url = (
                    f"{settings.AZDO_ORGANIZATION_URL.rstrip('/')}/_apis/identities"
                )
                resolve_params = {
...
async def get_thread_participants(
    ...
    """
    if not settings.AZDO_PAT or not threads_url:
        return []
    try:
        client = get_http_client()
...
async def resolve_identity(query: str | None) -> dict[str, str] | None:
    """Resolve an AzDO user identity by account name, unique name, or display name.
    ...
    """
    if not settings.AZDO_PAT or not settings.AZDO_ORGANIZATION_URL or not query:
        return None
    cleaned = query.strip()
    if not cleaned:
        return None
    ...
    try:
        url = f"{settings.AZDO_ORGANIZATION_URL.rstrip('/')}/_apis/identities"
        params = {
            "api-version": API_VERSION,
